...
const inputWrapperClassName = cn(
  "relative w-full rounded-xl bg-slate-100/90 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700/80 p-2.5 transition-all shadow-inner",
  "has-[:focus-visible]:border-blue-500 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-blue-400/20",
);

const inputClassName =
  "w-full bg-transparent outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-400 text-sm";

export type InputFieldProps = ComponentPropsWithoutRef<"input"> & {
  wrapperClassName?: string;
};

export type SmoothInputType = "text" | "password";
...
export const SmoothInput = React.forwardRef<HTMLInputElement, SmoothInputProps>(
  ...
  const displayPlaceholder = placeholder || "Type smoothly...";

  const syncMeasureSpan = () => {
    const input = inputRef.current;
    const measureSpan = measureRef.current;
    if (!input || !measureSpan || typeof window === "undefined") return;
  ...
  const measurePrefixWidth = (text: string) => {
    const input = inputRef.current;
    const measureSpan = measureRef.current;
    if (!input || !measureSpan || typeof window === "undefined") return null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div
          className={cn(
            "pointer-events-none col-start-1 col-end-2 row-start-1 row-end-2 h-[1.1em] w-[2.5px] rounded-full self-center shadow-[0_0_8px_rgba(59,130,246,0.6)]",
            caretColor
          )}
    ...
    <div className="flex h-full w-full flex-col items-center justify-center p-6 space-y-4">
      <div className="text-center space-y-1">
        ...
        <p className="text-xs text-slate-500">
          Spring-animated cursor tracks typed text with sub-pixel precision.
        </p>
      </div>
      <div className="w-full max-w-[420px] space-y-3">
        <SmoothInput placeholder="Try typing smoothly here..." aria-label="Smooth caret input" />
      </div>
    </div>
  );
};
