...
export const AIAssistantDrawer: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white/95 dark:bg-slate-900 text-slate-900 dark:text-slate-100 select-none overflow-hidden font-sans transition-colors">
      {/* Siri Apple Intelligence Clean Header */}
      <div className="relative p-3.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 flex-shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              <h3 className="text-xs font-bold text-slate-900 dark:text-white tracking-wide">AP Intelligence</h3>
              <span className="text-[9px] font-mono px-1.5 py-0.2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700">
                AP-AI
              </span>
            </div>
            <p className="text-[10px] text-slate-500 dark:text-slate-400">Trained on Anugamya's Portfolio & Engineering Work</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1.5">
          <button
            ...
            className={`p-1.5 rounded-lg border transition-colors ${
              voiceEnabled
                ? 'bg-fuchsia-500/20 border-fuchsia-500/40 text-fuchsia-600 dark:text-fuchsia-300'
                : 'bg-slate-100 dark:bg-white/5 border-slate-300 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'
            }`}
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-300 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors"
      ...
      {/* Chat Messages Feed */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700">
        {messages.map((msg) => {
              ...
              <div
                className={`w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md ${
                  isUser
                    ...
                    : 'bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-white/10 text-white'
                }`}
              >
                {isUser ? (
                  <User className="w-3.5 h-3.5" />
                ) : (
                  <img src="/icons/siri.png" alt="AP AI" className="w-5 h-5 object-contain" />
                )}
              </div>
                ...
                <div
                  className={`p-3 rounded-2xl text-xs leading-relaxed ${
                    isUser
                      ...
                      : 'bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-100 rounded-tl-xs backdrop-blur-md shadow-sm'
                  }`}
                >
                  <p className="whitespace-pre-line">{msg.text}</p>
                </div>

                <div className="flex items-center space-x-2 px-1 text-[10px] text-slate-400 dark:text-slate-500 font-mono">
        ...
        {isTyping && (
          <div className="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
            <div className="w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-white/10 flex items-center justify-center">
      ...
      <div className="px-3 py-1.5 overflow-x-auto flex space-x-2 flex-shrink-0 scrollbar-none border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-black/20">
        {QUICK_PROMPTS.map((prompt, idx) => (
          <button
            key={idx}
            onClick={() => handleSend(prompt)}
            className="px-2.5 py-1 rounded-full bg-white dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/15 border border-slate-200 dark:border-white/10 text-[11px] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white whitespace-nowrap transition-colors"
          >
            <span>{prompt}</span>
          </button>
        ))}
      ...
      <form
        ...
        className="p-3 border-t border-slate-200 dark:border-white/10 bg-white/95 dark:bg-slate-950 flex items-center space-x-2 flex-shrink-0 transition-colors"
      >
        <div className="flex-1">
          <SmoothInput
            ...
            wrapperClassName="bg-slate-100 dark:bg-white/10 border border-slate-300 dark:border-white/15 rounded-xl p-2"
            className="text-xs text-slate-900 dark:text-white placeholder:text-slate-400"
            caretColor="bg-fuchsia-500 shadow-[0_0_8px_rgba(232,121,249,0.8)]"
          />
        </div>
        <button
          type="submit"
