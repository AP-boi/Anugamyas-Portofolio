      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-700">
        ...
        {isTyping && (
          <div className="flex items-center space-x-2 text-xs text-slate-400">
            <div className="w-6 h-6 rounded-lg bg-slate-800 border border-white/10 flex items-center justify-center">
      ...
      <div className="px-3 py-1.5 overflow-x-auto flex space-x-2 flex-shrink-0 scrollbar-none border-t border-white/5 bg-black/20">
        {QUICK_PROMPTS.map((prompt, idx) => (
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-white/5 hover:bg-white/15 border border-white/10 text-[11px] text-slate-300 hover:text-white whitespace-nowrap transition-colors"
          >
            <span>{prompt}</span>
          </button>
        ))}
